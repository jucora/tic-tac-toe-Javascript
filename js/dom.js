...
function dialogBoxesPlayers(dialog, func, mode, dialogDetail) {
  document.getElementById('dialogboxbody').innerHTML = `<h2 id='namePlayerOneTitle'>${dialog}${dialogDetail}</h2>`;
  ...
  document.getElementById('dialogboxfoot').innerHTML = `<button class = 'btn btn-primary form-control' onclick="userInfo.ok('${
    func
  }')">OK`;
}

function selectGameMode(dialog) {
  document.getElementById('dialogboxbody').innerHTML = `<h2 id='nameInputTitle'>${dialog}</h2>`;
  document.getElementById('dialogboxbody').innerHTML
